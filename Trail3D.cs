    ...
    public void RenderTrail()
    {
        ...
        for (int j = 0; j < verts.Count - 1; j++)
        {
            ...
            for (int i = 0; i < densityAround; i++)
            {
                ...
                im.SurfaceAddVertex(cur[i]);
                im.SurfaceAddVertex(cur[nxtI]);
                im.SurfaceAddVertex(nxt[i]);
                im.SurfaceAddVertex(cur[nxtI]);
                im.SurfaceAddVertex(nxt[nxtI]);
                im.SurfaceAddVertex(nxt[i]);
            }
        }
        if (verts.Count > 1)
        {
            for (int i = 0; i < densityAround; i++)
            {
                ...
                im.SurfaceAddVertex(verts[0][i]);
                im.SurfaceAddVertex(Vector3.Zero);
                im.SurfaceAddVertex(verts[0][nxt]);
            }

            for (int i = 0; i < densityAround; i++)
            {
                var nxt = (i + 1) % densityAround;
                im.SurfaceAddVertex(verts[verts.Count - 1][i]);
                im.SurfaceAddVertex(verts[verts.Count - 1][nxt]);
                im.SurfaceAddVertex(lastP);
            }
        }
        im.SurfaceEnd();
    }
